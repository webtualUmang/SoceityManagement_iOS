...
open class SMSwipeableTabViewController: UIViewController, UIPageViewControllerDataSource, UIPageViewControllerDelegate, UIScrollViewDelegate {
    ...
    */
    ...
    //Fixed
    fileprivate lazy var segmentBarView = UIScrollView()
    ...
    open func pageViewController(_ pageViewController: UIPageViewController, didFinishAnimating finished: Bool, previousViewControllers: [UIViewController], transitionCompleted completed: Bool) {
        if completed {
            ...
            segmentBarView.scrollRectToVisible(CGRect(x: buttonsFrameArray[currentPageIndex].origin.x, y: 0.0,  width: buttonsFrameArray[currentPageIndex].size.width, height: 44.0), animated: true)
        }
    }
    ...
    @objc func didSegmentButtonTap(_ sender: UIButton) {
        ...
        pageViewController?.setViewControllers([viewControllerAtIndex(sender.tag)!], direction: scrollDirection, animated: true, completion: { (complete) -> Void in
            if complete {
                ...
                self.segmentBarView.scrollRectToVisible(CGRect(x: self.buttonsFrameArray[self.currentPageIndex].origin.x, y: 0.0,  width: self.buttonsFrameArray[self.currentPageIndex].size.width, height: self.segementBarHeight), animated: true)
            }
        })
    }
    ...
    open func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        selectionBar.frame = CGRect(x: xCoor-xFromCenter/kSelectionBarSwipeConstant, y: selectionBar.frame.origin.y, width: buttonsFrameArray[currentPageIndex].size.width, height: selectionBar.frame.size.height)
    }
}
