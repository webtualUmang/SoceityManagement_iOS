...
class CompainCloseViewController: UIViewController,UITableViewDataSource,UITableViewDelegate
{
        ...
        for data in resultList {
            if let tempData = data as? NSDictionary {
               ...
               // if let status = tempData.objectForKey("status") as? String {
                   // if status.caseInsensitiveCompare("Closed") == .OrderedSame {
...
//                                cell.lblTicketNo.textColor = UIColor.darkGray
    ...
    func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat{
        let cell = ComplainsCells.object(at: indexPath.row) as? ComplainsOpenCell
...
//            self.UpdateStatus(cell.data!)
    ...
    func UpdateStatus(_ ResultData : NSDictionary){
        let mutableData = NSMutableDictionary(dictionary: ResultData)
        mutableData.setObject("yes", forKey: kNoticeBoardisread as! NSCopying)
        //        print(mutableData)
        
        if let tempStr = ResultData.object(forKey: kkSocietyComplainsTablecomplaintID) as? String {
            MyDbManager.sharedClass().updateName(kSocietyComplainsTable, field: mutableData as! [AnyHashable: Any], whereField: [kkSocietyComplainsTablecomplaintID], fieldValue: [tempStr], or: false)
        }
    ...
    func GetNoticeBoard(_ pageCode : Int)
    {
        ...
        let urlStr = String(format: "%@?view=helpdesk_paginate&page=list&userID=%@&societyID=%@&pagecount=%d&complaint_type=Close", arguments: [kMainDomainUrl,appDelegate.LoginUserID, appDelegate.SocietyID,pageCode])
        print(urlStr)
