...
class BookedViewController: UIViewController,UITableViewDataSource,UITableViewDelegate,AddFacilityDelegate
{
    ...
    func GetNoticeBoard()
    {
        ...
        let urlStr = String(format: "%@?view=facility&page=open_booking_list&userID=%@&societyID=%@&count=0", arguments: [kMainDomainUrl,appDelegate.LoginUserID, appDelegate.SocietyID])
        print(urlStr)
        ...
        TNMWSMethod(nil, url: urlStr, isMethod: kGetMethod, AuthToken: "", viewController: self) { (succeeded, data) -> () in
            if succeeded == true {
                ...
                if let ResultData = data as? NSDictionary{
                    ...
                    if let msgCode = ResultData.object(forKey: "message") as? String {
                        if(msgCode.lowercased() == "no data found."){
