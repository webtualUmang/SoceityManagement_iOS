...
class MeetingClosedViewController: UIViewController,UITableViewDataSource,UITableViewDelegate
{
    ...
    func GetNoticeBoard(_ pageCode : Int)
    {
        ...
        let urlStr = String(format: "%@?view=meeting_paginate&page=list&userID=%@&societyID=%@&pagecount=%d&filter=%@&meeting_type=Close", arguments: [kMainDomainUrl,appDelegate.LoginUserID, appDelegate.SocietyID,pageCode,self.fillterStr])
        print(urlStr)
