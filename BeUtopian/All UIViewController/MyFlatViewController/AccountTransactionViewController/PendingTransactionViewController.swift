...
class PendingTransactionViewController: UIViewController, PendingTransactionCellDelegate,UITableViewDelegate,UITableViewDataSource {
    ...
    override func viewDidLoad() {
        ...
        GetPendingList(appDelegate.FlatID)
    ...
    func BtnPayAmmount(_ cell: PendingTransactionCell) {
      ...
      //  let rootObj: BankDetailsViewController = BankDetailsViewController(nibName: "BankDetailsViewController", bundle: nil)
        let storyboard = UIStoryboard(name: "Main", bundle: nil)
        let rootObj:BankDetailsViewController = storyboard.instantiateViewController(withIdentifier: "BankDetailsViewController") as! BankDetailsViewController
        
        if let tempStr = cell.data!.object(forKey: "incomeID") as? String {
            rootObj.invoiceID = tempStr
        }
    ...
    func GetPendingList(_ flatId : String){
        
      
        
        let urlStr = String(format: "%@?view=income_invoice&page=pending&userID=%@&societyID=%@&flatID=%@&pagecode=0", arguments: [kMainDomainUrl,appDelegate.LoginUserID, appDelegate.SocietyID,flatId])
        print(urlStr)
                ...
                if let ResultData = data as? NSDictionary{
                    if let msgCode = ResultData.object(forKey: "msgcode") as? String {
                        if msgCode == "0" {
                            ...
                            if let bannerList = ResultData.object(forKey: "pending_list") as? NSArray {
                                DispatchQueue.main.async {
