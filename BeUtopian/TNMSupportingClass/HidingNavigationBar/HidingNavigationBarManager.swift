...
open class HidingNavigationBarManager: NSObject, UIScrollViewDelegate, UIGestureRecognizerDelegate {
	...
	fileprivate var contractionResistance: CGFloat = 0
	
	open var delegate: HidingNavigationBarManagerDelegate?
	...
	public init(viewController: UIViewController, scrollView: UIScrollView){
		...
		navBarController.expandedCenter = {(view: UIView) -> CGPoint in
			return CGPoint(x: view.bounds.midX, y: view.bounds.midY + self.statusBarHeight())
		}
