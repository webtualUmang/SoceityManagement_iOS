...
typedef enum
{
    TNMKeyboardControlPreviousNext = 1 << 0,
    TNMKeyboardControlDone = 1 << 1
} TNMKeyboardControl;
...
typedef enum
{
    TNMKeyboardControlsDirectionPrevious = 0,
    TNMKeyboardControlsDirectionNext
} TNMKeyboardControlsDirection;

@protocol TNMKeyboardControlsDelegate;

@interface TNMKeyboardControls : UIView

/**
 *  Delegate to send callbacks to.
 */
@property (nonatomic, strong) id <TNMKeyboardControlsDelegate> delegate;

/**
 *  Visible controls. Use a bitmask to show multiple controls.
 */
@property (nonatomic, assign) TNMKeyboardControl visibleControls;
...
@protocol TNMKeyboardControlsDelegate <NSObject>
@optional
/**
 ...
 */
- (void)keyboardControls:(TNMKeyboardControls *)keyboardControls selectedField:(UIView *)field inDirection:(TNMKeyboardControlsDirection)direction;
/**
 *  Called when the done button was pressed.
 *  @param keyboardControls The instance of keyboard controls.
 */
- (void)keyboardControlsDonePressed:(TNMKeyboardControls *)keyboardControls;
@end