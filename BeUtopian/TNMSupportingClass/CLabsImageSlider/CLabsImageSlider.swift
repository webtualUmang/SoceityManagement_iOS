...
@objc public protocol updateUI {
    func imageUpdate(_ index:Int)
}



open class CLabsImageSlider:UIView,updateUI
{
    ...
    open var sliderDelegate  :   imageSliderDelegate?
   ...
   open func setUpView(_ imageSource:imageSrc,slideType:slideCase,isArrowBtnEnabled:Bool)
    {
        switch imageSource{
        ...
        case .automatic(let timer):
              ...
              timeInterval = Timer.scheduledTimer(timeInterval: timer, target: self, selector: #selector(CLabsImageSlider.moveRight), userInfo: nil, repeats: true)
   ...
   @objc  func moveLeft()
    ...
    @objc  func swipeLeft(_ swipeGesture:UISwipeGestureRecognizer)
    {
     ...
     func swipeLeftAnimate(_ condition:Int)
    {
        isAnimating=true
        switch condition {
        case 1:
   ...
   @objc  func swipeRight(_ swipeGesture:UISwipeGestureRecognizer)
    {
     ...
     func swipeRightAnimate(_ condition:Int){
        isAnimating=true
        switch condition {
        case 1:
            self.visibleImageView    =   self.imageView2
     ...
     func prepareImageSource(_ images:[String]){
     ...
     func startImageDownload(_ index:Int)
    {
    ...
    open func imageUpdate(_ index:Int)
    {
...
open class imageData {
    ...
    var imageUrl    :   String
    var isloading   =   Bool()
    open weak var delegate : updateUI?
