...
@inline(__always) func AnimationOptionsForCurve(_ curve: UIView.AnimationCurve) -> UIView.AnimationOptions {
...
extension UIView: UIGestureRecognizerDelegate {
    ...
    fileprivate func addKeyboardControl(panning: Bool, frameBasedActionHandler: TKKeyboardDidMoveBlock?, constraintBasedActionHandler: TKKeyboardDidMoveBlock?) {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(UIView.responderDidBecomeActive(_:)), name: UITextField.textDidBeginEditingNotification, object: nil)
        
        NotificationCenter.default.addObserver(self, selector: #selector(UIView.responderDidBecomeActive(_:)), name: UITextView.textDidBeginEditingNotification, object: nil)
        
        // Register for keyboard notifications
        NotificationCenter.default.addObserver(self, selector: #selector(UIView.inputKeyboardWillShow(_:)), name: UIResponder.keyboardWillShowNotification, object: nil)
        
        NotificationCenter.default.addObserver(self, selector: #selector(UIView.inputKeyboardDidShow), name: UIResponder.keyboardDidShowNotification, object: nil)
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(UIView.inputKeyboardWillHide(_:)), name: UIResponder.keyboardWillHideNotification, object: nil)
        
        NotificationCenter.default.addObserver(self, selector: #selector(UIView.inputKeyboardDidHide), name: UIResponder.keyboardDidHideNotification, object: nil)
    }
    ...
    @objc fileprivate func inputKeyboardWillShow(_ notification: Notification) {
        ...
        guard let keyboardEndFrameWindow: CGRect = ((userInfo[UIResponder.keyboardFrameEndUserInfoKey] as? NSValue)?.cgRectValue) else { return }

        guard let keyboardTransitionDuration: Double = userInfo[UIResponder.keyboardAnimationDurationUserInfoKey] as? Double else { return }
        
        var keyboardTransitionAnimationCurve: UIView.AnimationCurve = .easeInOut
        if let curveIntValue = (userInfo[UIResponder.keyboardAnimationCurveUserInfoKey] as? NSNumber)?.intValue,
           let curve = UIView.AnimationCurve(rawValue: curveIntValue << 16) {
